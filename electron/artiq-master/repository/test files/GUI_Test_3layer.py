...
class Electron(HasEnvironment):
    def build(self):
        ...
        self.setattr_device("ttl12") # sync with aom ttl8 pulse in run_outputting mode
    ...
    def prepare(self):
        ...
        # self.set_dataset(key="optimize.e.tg", value=np.float32(0), broadcast=True, persist=True)
        ...
        self.pin_matching = {"bl1":5,"bl2":11,"bl3":25,"bl4":24,"bl5":23,"br1":6,"br2":20,"br3":8,"br4":22,"br5":12,"tl1":2,"tl2":1,"tl3":10,"tl4":15,"tl5":3,"tr1":18,"tr2":19,"tr3":9,"tr4":17,"tr5":4,"tg":14,"trigger_level":0}
        self.gnd = [7,13,16,21] # gnd pins
        ...
        # self.controlled_electrodes = ["tl2","tl3","tl4","tr2","tr3","tr4","trigger_level"]
        # self.controlled_multipoles = ["Grid","Ex","Ey","Ez","U2","U3","U4"]
    ...
    def setupUi(self, win):
        self.title = 'Electron GUI 3-layer'
        self.left = 0
        self.top = 0
        self.width = 1200 # 600
        self.height = 600 # 200
...
class MyTabWidget(HasEnvironment,QWidget):
    ...
    def setup_UI(self):
        ...
        #tg
        spin_tg = QtWidgets.QDoubleSpinBox(self)
        spin_tg.setDecimals(4)
        spin_tg.setRange(-25,25)
        spin_tg.setSingleStep(0.1)
        # spin_tg.setValue(self.default_voltages[-2])
        grid1.addWidget(spin_tg,1,3,1,1)
        self.electrode_spin[self.ELECTRODES[2][0]] = spin_tg
        # self.electrodes.append(spin_t0)
        label_tg = QLabel('       '+self.ELECTRODES[2][0], self)
        label_tg.setAlignment(QtCore.Qt.AlignRight)
        grid1.addWidget(label_tg,1,2)
        ...
        label_tg = QLabel('       '+self.ELECTRODES[2][0], self)
        label_tg.setAlignment(QtCore.Qt.AlignRight)
        grid4.addWidget(label_tg,1,2,1,1)
        self.label0_tg = QLabel(str(self.default_voltages["tg"]), self)
        self.label0_tg.setStyleSheet("border: 1px solid black;")
        grid4.addWidget(self.label0_tg,1,3,1,1)
        self.electrode_labels[self.ELECTRODES[2][0]] = self.label0_tg
        ...
        # MULTIPOLES = ['Grid: (V)','Ex:', 'Ey:', 'Ez:', 'U1:', 'U2:', 'U3:', 'U4:', 'U5:']
        # self.multipoles = []
