...
class DAC_Control(EnvExperiment, object):
#class DAC_Control():
    
    #create maximum voltage +/- 10
  
    def build(self):
        pass
    
    def prepare(self):
        self.set_dataset(key="dac_voltages", value=np.full(21,0), broadcast=True, persist=self.persist)
    ...
    def run(self):
        ...
        print("test:", self.voltages)        

        
        #self.value = 'tl1'
        #self.key = voltages['tl1']
        self.mutate_dataset(key="dac_voltages", index=0, value=voltages['tl1'])
